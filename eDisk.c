...
// **write this function**
	...
	uint8_t *sectorPtr = (uint8_t*) (EDISK_ADDR_MIN + 512 * sector); // cast absolute address the byte pointer
...
// **write this function**
	...
	// set writeAddr (must be 128-byte aligned 0x80)
	uint32_t writeAddr = EDISK_ADDR_MIN + 512*sector;
			
	if( writeAddr > EDISK_ADDR_MAX)
		return RES_PARERR;
			
	// convert buff pointer to 32-bit pointer (this will be source for writing)
	uint32_t *buff32 = (uint32_t*)buff;
	
	for(int i=0; i<128; i+=32){
		Flash_FastWrite( (buff32+i), writeAddr + 4*i, 32); // write 128 32-bit values to flash memory (32 at a time)
	}	
...
// **write this function**
	...
	uint32_t addr = EDISK_ADDR_MIN;

	while( addr < EDISK_ADDR_MAX){
		if(Flash_Erase(addr))
			return RES_ERROR;	// error erasing data
